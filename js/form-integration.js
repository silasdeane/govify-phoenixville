...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const checkForFormPortal = function(data) {
      if (data && data.result && data.result.includes('<form_portal>')) {
        ...
        // Add a message and iframe in a light background container
        formContainer.innerHTML = `
          ...
          <div style="margin-top: 16px;">
            ...
            ">
              <iframe 
                src="/static/${formData.form_id}.html" 
                width="100%" 
                height="600" 
                style="
                  border: none;
        ...
        // Add a force light mode class to the iframe when it loads
        const iframe = formContainer.querySelector('iframe');
        iframe.onload = function() {
          try {
            // Try to access the iframe content and force light mode
            ...
            // Add a light theme class and inline style
            iframeBody.style.backgroundColor = '#f9f9f9';
            iframeBody.style.color = '#333';
            
            // Find any dark-theme related elements and force light mode
            const darkElements = iframeDoc.querySelectorAll('.dark-theme, [class*="dark-"], [style*="background-color: #121212"]');
    ...
    // Add CSS for form action buttons and iframe styling
    ...
    style.textContent = `
      ...
      /* Force light background for iframes */
      iframe {
        background-color: white !important;
      }
      
      /* Extra wrapper to ensure iframe has light background */
      .iframe-light-wrapper {
        background-color: white !important;
